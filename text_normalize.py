...
def normalize_text(text):
    ...
    df["after"] = df["previous"].apply(
        lambda m: normalize_single(m.split("|")[1], m.split("|")[0], m.split("|")[2])
    )

    return (
        TreebankWordDetokenizer()
        .detokenize(df["after"].tolist())
        .replace("’ s", "'s")
        .replace(" 's", "'s")
    )
